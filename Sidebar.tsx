...
const Sidebar: React.FC<SidebarProps> = ({ 
  isOpen, functions, params, view, activeAnalyses, calculatedPoints, editingObject, setEditingObject, contextMenu, setContextMenu,
  ...
  const getAnalysisLabel = (an: AnalysisObject) => {
    if (an.type === 'polygon') return `\\text{Polígono } (${an.vertices?.length || 0} \\text{ vértices})`;
    if (an.type === 'freePoint') return `\\text{Ponto } ${an.label || ''}`;
    ...
    if (an.type === 'tangent_analysis') {
         ...
         return eq ? `${label}: ${eq}` : `\\text{Reta Tangente } ${label}`;
    }
    if (an.type === 'line' && an.vertices && an.vertices.length === 2) {
        ...
        return `\\text{Reta } ${an.label || ''}`;
    }
    if (an.type === 'ray') return `\\text{Semirreta}`;
    if (an.type === 'segment') return `\\text{Segmento de Reta}`;
    if (an.type === 'func_analysis') return `\\text{Ponto(s) em Função} (${an.subtype || ''}) ${an.label || ''}`;
    if (an.type === 'intersection_analysis') return `\\text{Interseção } ${an.label || ''}`;
    if (an.type === 'slope_analysis') return `\\text{Declive}`;
    return `\\text{Objeto Geométrico}`;
  }
  ...
  return (
    <div className={`${isOpen ? 'w-80 border-r border-[#E5E7EB]' : 'w-0'} bg-white flex flex-col transition-all duration-300 overflow-hidden relative shadow-lg z-10 h-full`}>
      <div className="flex-1 overflow-y-auto p-4 space-y-4 pb-16"> 
          {trueFunctions.map(f => {
            ...
            <div className={`group border border-gray-200 hover:border-[#1CA6B2] p-3 rounded bg-gray-50 flex flex-col gap-2 transition-colors ${editingObject?.id === f.id && editingObject?.type === 'function' ? 'ring-2 ring-[#007888] border-[#007888]' : ''}`}> 
                <div className="flex items-center gap-2"> 
                    <div className="w-8 h-8 cursor-pointer flex items-center justify-center hover:bg-gray-200 rounded transition-colors flex-shrink-0" onClick={() => setEditingObject({id: f.id, type: 'function'})} title="Propriedades">
                ...
                {hasParamsInExpr && substitutedLatex && (
                  <div className="flex items-center gap-2 pl-8 border-t border-gray-100 pt-2 mt-1">
                    <span className="text-[10px] uppercase font-bold text-gray-400 whitespace-nowrap">Editar:</span>
          ...
          <div className="border-t border-[#E5E7EB] pt-4 mt-2">
              ...
              <div key={f.id} className={`mb-2 group border border-gray-200 hover:border-[#1CA6B2] p-3 rounded bg-gray-50 flex flex-col gap-1 transition-colors ${editingObject?.id === f.id && editingObject?.type === 'function' ? 'ring-2 ring-[#007888] border-[#007888]' : ''}`}>
                  ...
                  {hasParamsInExpr && substitutedLatex && (
                    <div className="flex items-center gap-1 pl-6 pt-1">
                      <div className="flex-1 bg-white rounded border border-gray-100 px-1">
                         <MathInput value={f.inputValue} onChange={(val) => onUpdateFunction(f.id, val)} onEnter={(val) => onFunctionSubmit(f.id, val)} className="text-[10px] py-0 h-5" />
                      </div>
                    </div>
                  )}
              </div>
            ...
            {activeAnalyses.length === 0 && algebraicConstructions.length === 0 && <div className="text-gray-400 text-xs italic">Nenhum objeto construído.</div>}
            {activeAnalyses.map(an => {
                ...
                // Ocultar da lista se for pointOn e estiver fora do domínio (não calculado)
                         ...
                         {editingCoords?.id === aidStr ? (
                            ...
                            (an.type === 'intersection_analysis' || an.type === 'func_analysis') ? (
                                <div className="flex flex-col w-full">
                                    <span className="text-sm font-bold text-gray-700"><Latex>{`\\text{${an.type === 'intersection_analysis' ? (an.label || 'Interseção') : (an.subtype === 'zeros' ? 'Raízes' : (an.subtype === 'max' ? 'Máximos' : 'Mínimos'))}}`}</Latex></span>
                                    {derivedPoints.length > 0 ? (derivedPoints.map(pt => (<div key={pt.id} className="text-sm text-gray-600 pl-1 mt-0.5"><Latex>{pt.label || 'P'}</Latex></div>))) : (<span className="text-[10px] text-gray-400 italic pl-1">A calcular...</span>)}
                                </div>
                            ) : (
                                isPointOnFunction ? (
                                    <div className="flex items-center gap-1 text-sm text-gray-700 font-medium cursor-text hover:bg-gray-100 rounded px-1 transition-colors" onClick={(e) => startEditingCoords(e, an)} title="Clique para editar"><Latex>{getAnalysisLabel(an)}</Latex></div>
                                ) : an.type === 'freePoint' && currentX !== undefined ? (
                                    <div className="flex items-center gap-0.5 text-sm text-gray-700 font-medium cursor-text hover:bg-gray-100 rounded px-1 transition-colors" onClick={(e) => startEditingCoords(e, an)} title="Clique para editar"><Latex>{`${an.label || 'P'}(${formatNumberDecimal(currentX, precision)}, ${formatNumberDecimal(currentY || 0, precision)})`}</Latex></div>
                                ) : (
                                    an.type === 'slope_analysis' ? (
                                        ...
                                        <span className={`text-sm font-medium text-gray-700 ${(an.type === 'line') ? 'hover:bg-gray-200 rounded px-1 cursor-text' : ''}`} onDoubleClick={(e) => startEditingCoords(e, an)} title={(an.type === 'line') ? "Duplo clique para editar" : ""}><Latex>{getAnalysisLabel(an)}</Latex></span>
                                    )
                                )
                            )
                         )}
      ...
      {(contextMenu || (editingObject && targetObj)) && (
          ...
          <div className="fixed z-50 bg-white rounded-lg shadow-xl border border-gray-200 p-4 w-72 overflow-y-auto max-h-[85vh] transition-all" style={contextMenu ? { top: contextMenu.y, left: contextMenu.x } : { top: 80, right: rightSidebarOpen ? 340 : 20 }}>
            <div className="flex justify-between items-center mb-4 border-b pb-2">
                <h4 className="text-sm font-bold uppercase text-[#007888] tracking-wider">Propriedades</h4>
            ...
            <div className="space-y-5">
                <div>
                  <label className="block text-[11px] font-bold text-gray-500 mb-2 uppercase tracking-tighter">Cor</label>
                ...
                <div>
                  <label className="block text-[11px] font-bold text-gray-500 mb-2 uppercase tracking-tighter">Visibilidade & Rasto</label>
                  <div className="flex gap-2">
                    <button onClick={() => onUpdateObjectStyle({ visible: !targetObj.visible })} className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all shadow-sm ${targetObj.visible !== false ? 'bg-[#E0F2F1] text-[#007888] border border-[#B2DFDB]' : 'bg-gray-100 text-gray-600 border border-gray-200'}`}>{targetObj.visible !== false ? 'Visível' : 'Oculto'}</button>
                    <button onClick={() => onUpdateObjectStyle({ trace: !targetObj.trace })} className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all shadow-sm ${targetObj.trace ? 'bg-indigo-100 text-indigo-700 border border-indigo-200' : 'bg-gray-100 text-gray-600 border border-gray-200'}`}>{targetObj.trace ? 'Rasto On' : 'Rasto Off'}</button>
                  </div>
                </div>
                ...
                {(targetObj.type === 'function' || ['line', 'ray', 'segment', 'polygon', 'tangent_analysis', 'inequality', 'vertical', 'implicit'].includes(targetObj.type as any)) && (
                  <div className="space-y-4 pt-2 border-t border-gray-100">
                    <div>
                      <label className="block text-[11px] font-bold text-gray-500 mb-2 uppercase tracking-tighter">Estilo de Traço</label>
                      <div className="flex gap-1.5">
                        {['solid', 'dashed', 'dotted'].map(s => (
                          <button key={s} onClick={() => onUpdateObjectStyle({ lineStyle: s })} className={`flex-1 py-1.5 rounded-md border text-[10px] uppercase font-bold transition-all ${targetObj.lineStyle === s || (s === 'solid' && !targetObj.lineStyle) ? 'bg-blue-50 border-blue-300 text-blue-700' : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-50'}`}>{s === 'solid' ? 'Cheio' : (s === 'dashed' ? 'Traço' : 'Ponto')}</button>
                        ))}
                      </div>
                    </div>
                    <div>
                      <div className="flex justify-between items-center mb-1.5">
                        <label className="block text-[11px] font-bold text-gray-500 uppercase tracking-tighter">Espessura</label>
                ...
                {['freePoint', 'pointOn_analysis', 'intersection_analysis', 'func_analysis'].includes(targetObj.type as any) && (
                  <div className="space-y-4 pt-2 border-t border-gray-100">
                    <div>
                      <label className="block text-[11px] font-bold text-gray-500 mb-2 uppercase tracking-tighter">Estilo de Ponto</label>
                      <div className="flex gap-1.5">
                        {['solid', 'open', 'cross'].map(s => (
                          <button key={s} onClick={() => onUpdateObjectStyle({ pointStyle: s })} className={`flex-1 py-1.5 rounded-md border text-[10px] uppercase font-bold transition-all ${targetObj.pointStyle === s || (s === 'solid' && !targetObj.pointStyle) ? 'bg-blue-50 border-blue-300 text-blue-700' : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-50'}`}>{s === 'solid' ? 'Cheio' : (s === 'open' ? 'Vazio' : 'Cruz')}</button>
                        ))}
                      </div>
                    </div>
                    <div>
                      <div className="flex justify-between items-center mb-1.5">
                        <label className="block text-[11px] font-bold text-gray-500 uppercase tracking-tighter">Tamanho</label>
